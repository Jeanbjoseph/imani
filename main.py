...
import azure_ia  # Já com configure_azure(...) e extrair_recomendacoes_ia(...)

# ================================
# Configuração inicial do Streamlit
# ================================
st.set_page_config(page_title="Analisador IA + Azure Blob via .env", layout="wide")
...
# ===================================
# 2. Função auxiliar para parsear o .env
# ===================================
def parse_dotenv(texto: str) -> dict[str, str]:
    ...
    for raw_line in texto.splitlines():
        ...
        vars_dict[chave.strip()] = valor.strip()
    return vars_dict
...
# ===================================
with st.sidebar:
    st.header("1. Faça upload do seu arquivo `.env`")
    env_file = st.file_uploader("Upload do `.env` (extensão .env)", type=["env"])

if not env_file:
    st.info("🚩 Por favor, faça o upload do arquivo `.env` na barra lateral para continuar.")
    st.stop()

try:
    conteudo_env = env_file.getvalue().decode("utf-8")
except Exception as e:
    st.error(f"❌ Não foi possível ler o `.env`: {e}")
    st.stop()

env_vars = parse_dotenv(conteudo_env)

# ===================================
# 4. Validação das chaves obrigatórias no .env
# ===================================
chaves_obrigatorias = [
...
faltando = [c for c in chaves_obrigatorias if c not in env_vars]
if faltando:
    st.error(f"❌ Variáveis obrigatórias faltando no .env: {', '.join(faltando)}")
    st.stop()

# ===================================
# 5. Configurar Azure OpenAI
# ===================================
azure_endpoint = env_vars["AZURE_OPENAI_ENDPOINT"].rstrip("/")
...
# ===================================
account_url = env_vars["BLOB_ACCOUNT_URL"].rstrip("/")  # ex: https://<conta>.blob.core.windows.net
container_name = env_vars["BLOB_CONTAINER_NAME"]
auth_method = env_vars["BLOB_AUTH_METHOD"].lower()

try:
    if auth_method == "service_principal":
        client_id = env_vars.get("BLOB_CLIENT_ID", "")
        client_secret = env_vars.get("BLOB_CLIENT_SECRET", "")
        tenant_id = env_vars.get("BLOB_TENANT_ID", "")
        if not client_id or not client_secret or not tenant_id:
            raise KeyError("BLOB_CLIENT_ID, BLOB_CLIENT_SECRET e BLOB_TENANT_ID devem estar no `.env`.")
        ...
        username = env_vars.get("BLOB_USERNAME", "")
        password = env_vars.get("BLOB_PASSWORD", "")
        tenant_id = env_vars.get("BLOB_TENANT_ID", "")
        if not username or not password or not tenant_id:
            raise KeyError("BLOB_USERNAME, BLOB_PASSWORD e BLOB_TENANT_ID devem estar no `.env`.")
        blob_credential = UsernamePasswordCredential(
            username=username,
            password=password,
            tenant_id=tenant_id
        )
    ...
    st.error(f"❌ Chave obrigatória faltando no `.env`: {err_key}")
    st.stop()
except Exception as e:
